...
async function getMelodies() {
    ...
    const ref = await (
        await fetch(
            `${dbInfoStore.url}/user-melody/${userInfoStore.userUUID}`,
            requestOptions,
        )
    ).json();
    if (!ref.error) {
...
async function removeFromPlaylist() {
    ...
    for (const userMelodyUUID of checkedPieces.value) {
        ...
        try {
            await fetch(
                `${dbInfoStore.url}/user-melody/${userInfoStore.userUUID}/${userMelodyUUID}`,
                requestOptions,
            );
        } catch (error) {
            console.error(
                `Failed to delete piece with UUID ${userMelodyUUID}`,
                error,
            );
        }
    }
        ...
        <Backdrop>
            ...
            <MyBorder class="m-2.5">
                <ButtonBar class="block m-2.5" v-slot="{ counter }">
                    ...
                    >
                    <ButtonItem :counter="counter()" group-name="melody-status"
                        >For Later</ButtonItem
                    >
                    <ButtonItem :counter="counter()" group-name="melody-status"
                        >Completed</ButtonItem
                    >
                </ButtonBar>
                <div
                ...
                >
                    <Checkbox
                        ...
                        :inputId="`${melody.pieceUUID}`"
                        name="thumbnail"
                        :value="`${melody.userMelodyUUID}`"
                    />
                    <MyBorder class="p-1">
                        <label :for="`${melody.pieceUUID}`">
                            <ThumbnailVideo
                                :embeddedURL="`${melody.url.split(/(\?v=)/g)[2]}`"
                            ></ThumbnailVideo>
                        </label>
                        <p>{{ melody.name }}</p>
                ...
                <button
                    class="block p-1 m-2.5 text-1 font-bold bg-white/30 hover:bg-white/40"
