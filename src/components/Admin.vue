...
async function getUsers() {
    ...
    const res = await (
        await fetch(`${dbInfoStore.url}/user/all`, requestOptions)
    ).json();
...
async function updateAccessLevel(userUUID, newLevel) {
    ...
    if (userUUID === userInfoStore.userUUID) {
        wantsToLowerOwnPermission = confirm(
            "Are you sure you want to lower your permissions?",
        );
    }
    ...
    if (userUUID === userInfoStore.userUUID && wantsToLowerOwnPermission) {
        const res = await (
            await fetch(
                `${dbInfoStore.url}/user/${userUUID}/access`,
                requestOptions,
            )
        ).json();
...
async function deleteUser(user) {
    ...
    const res = await (
        await fetch(`${dbInfoStore.url}/user/${user.userUUID}`, requestOptions)
    ).json();
    if (!res.error) {
    ...
    <Background>
        <Backdrop>
            ...
            <MyBorder v-for="user in users" :key="user.userUUID">
                ...
                <ButtonBar>
                    <ButtonItem
                        :group-name="user.userUUID"
                        :counter="1"
                        :is-selected="user.accessLevel === 'read'"
                        @click="updateAccessLevel(user.userUUID, 'read')"
                        >Read</ButtonItem
                    >
                    <ButtonItem
                        :group-name="user.userUUID"
                        :counter="2"
                        :is-selected="user.accessLevel === 'write'"
                        @click="updateAccessLevel(user.userUUID, 'write')"
                        >Write</ButtonItem
                    >
                    <ButtonItem
                        :group-name="user.userUUID"
                        :counter="3"
                        :is-selected="user.accessLevel === 'admin'"
                        @click="updateAccessLevel(user.userUUID, 'admin')"
                        >Admin</ButtonItem
                    >
