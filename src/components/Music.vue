...
async function getAllMelodies() {
    ...
    let response = await (
        await fetch(`${dbInfoStore.url}/melody`, requestOptions)
    ).json();
    if (!response.error) {
...
function filterMelodies() {
    ...
    for (let i = 0; i < allMelodies.value.length; i++) {
        ...
        const isInLengthRange =
            length > range.value[0] && length < range.value[1];
        const isInGenre = selectedGenres.value.includes(
            melody.genre.toLowerCase(),
        );
